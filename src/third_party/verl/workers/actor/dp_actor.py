...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        select_keys = ['responses', 'input_ids', 'attention_mask', 'gen_mask', 'position_ids', 'old_log_probs', 'advantages']
        if self.config.use_kl_loss:
            select_keys.append('ref_log_prob')
        data_subset = data.select(batch_keys=select_keys).batch
        has_multi_modal_inputs = 'multi_modal_inputs' in data.non_tensor_batch.keys()
        ...
        if has_multi_modal_inputs:
            ...
            num_mini_batches = data.batch.batch_size[0] // self.config.ppo_mini_batch_size
            dataloader = data_subset.split(self.config.ppo_mini_batch_size)
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    clip_ratio_min = self.config.clip_ratio_min
                    clip_ratio_max = self.config.clip_ratio_max
